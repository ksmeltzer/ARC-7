...
# We use Node.js to safely parse and merge the JSON to avoid adding dependencies like jq,
# since Node is guaranteed to be installed for Opencode environments.
node -e "
const fs = require('fs');
...
try {
    ...
    if (updated) {
        ...
        console.log('ℹ️ Agent mappings already exist in $CONFIG_FILE. No changes made.');
    }
} catch (err) {
