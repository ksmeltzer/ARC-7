...
if ($Mode -eq "symlink") {
    try {
        ...
        $null = New-Item -ItemType Junction -Force -Path "$TargetDir\agent\ARC-7" -Target "$RepoRoot\agents\ARC-7"
    ...
    Copy-Item -Recurse -Force "$RepoRoot\agents\ARC-7" "$TargetDir\agent\"
    Write-Host "✅ Copied ARC-7 to $TargetDir" -ForegroundColor Green
}

Write-Host "`nNext steps:" -ForegroundColor Yellow
Write-Host "  1. Run .\configure_models.ps1 -Provider <your_provider> to map your models."
